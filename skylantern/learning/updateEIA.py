...
def updateEIA(s):
    ...
    for sid in _EIA_storage + _EIA_price:
        time.sleep(5)
        try:
            df = fetchEIA(sid, update=True)
            if not df.empty:
                if sid in _EIA_price:
                    bulk_save(s, map_eia_price(df, sid))
                elif sid in _EIA_storage:
                    bulk_save(s, map_eia_storage(df, sid))
            logger.debug('(EIA %s) %s entire(s) written' % ( sid, len(df.index) ))
